...
def runstrat(args=None):
    ...
    data = pd.read_pickle(args.data0)
    data = data["2020-1":"2021-12"]
    ic(data.head())
    ...
    if args.plot:
        pkwargs = dict(style='candle', iplot=False, volume=False, width=16, height=10)
...
def parse_args(pargs=None):
    ...
    parser = argparse.ArgumentParser(
        formatter_class=argparse.ArgumentDefaultsHelpFormatter, description="Sample for sizer"
    )

    parser.add_argument(
        "--data0",
        required=False,
        default="data/현대차.pkl",
    ...
    parser.add_argument(
        "--todate", required=False, default="2006-12-31", help="Ending date in YYYY-MM-DD format"
    )
...
if __name__ == "__main__":
    ...
    # cerebro.addstrategy(TALibStrategy, ind="sma", doji=True)
    # cerebro.run(runcone=True, stdstats=False)
    # if args.plot:
